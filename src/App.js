...
export default class App extends React.Component {
  ...
  matchCards=(c1,c2,c3,symbol)=>{
    return (this.cards[c1].symbol == symbol &&
    this.cards[c2].symbol == symbol &&
    this.cards[c3].symbol == symbol);
  }
  checkTie=()=>{
    let allCards = [0,1,2,3,4,5,6,7,8];
    return allCards.every((val)=>{return this.cards[val].symbol != ''});
  }
  checkWinner = symbol => {
    if (this.matchCards(0,1,2,symbol)|| this.matchCards(3,4,5,symbol) || this.matchCards(6,7,8,symbol)||this.matchCards(0,3,6,symbol)||this.matchCards(1,4,7,symbol)||this.matchCards(2,5,8,symbol)||this.matchCards(0,4,8,symbol)||this.matchCards(2,4,6,symbol)) {
      if(status == 1 ){
  ...
  render() {
    return (
      <div id='container'>
        <Player stat={status} playerName={this.state.player} playerIndex={0} changePlayerName={this.handlePlayerName}/>
          <div id="game">
          
            <h1>Tic-Tac-Toe</h1>
            <hr />
            <Menu handleClick={this.handelStart} playerName={this.state.player} changePlayerName={this.handlePlayerName}/>
              ...
              {this.state.cardList.map((card, index) => {
                return (
                  <Card
                    ...
                    symbol={card.symbol}
                    checkresult ={()=>{this.checkWinner('close');}}
                  />
                );
              })}
            </div>
